    ...
    var that = {
      ...
      redraw:function(){
        ...
        particleSystem.eachNode(function(node, pt){
          ...
          // draw the text
          if (label){
            ctx.font = "20px bold 'Avenir LT Std', sans-serif"
      ...
      initMouseHandling:function(){
        ...
        var handler = {
          ...
          dragged:function(e){
            ...
            $(canvas).unbind('mouseup', handler.mypersonalfunction)
            return false
          },
          ...
          mypersonalfunction:function(e){
               if (dragged===null || dragged.node===undefined) return
                if (dragged.node !== null){
                    ...
                    //alert('Node selected: ' + id + $("#node"+id+"Data").attr("dataArtistes") + dragged.node.data.link);
                    var nom = dragged.node.data.link;
                    if( dragged.node.data.alpha === 0){
                    popUpArtiste(nom);
                    }else if( dragged.node.data.alpha === 1 ){
                    popUpMouvement(nom);
                    }
                }
                $(canvas).unbind('mousemove', handler.dragged)
